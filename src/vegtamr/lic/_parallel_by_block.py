...
def _calculate_block_size(array_shape, streamlength):
  ...
  try:
    ...
    print(f"Block size calculation failed: {error}")
    return None
...
def _generate_chunks(array_shape, streamlength):
  ...
  if (block_size is None) or (array_shape[0] < 2*block_size):
    print(f"Using row chunking (streamlength={streamlength})")
    return [
      ("row", row_index)
      for row_index in range(array_shape[0])
    ]
  print(f"Using block chunking {block_size}x{block_size} (streamlength={streamlength})")
  chunks = []
